...
/**
 * True when the event carries a score (submit), e.g. ASSESS with edata.score
 * or summary score. `isScorm` selects the coercion rule — SCORM content
 * reports its score as a string (`"95"`), which a strict `typeof === 'number'`
 * check would miss.
 */
export function eventHasScore(event: TelemetryEvent | undefined, isScorm: boolean): boolean {
  ...
  const summary = (rawData?.edata as { summary?: unknown } | undefined)?.summary ?? (rawData as { summary?: unknown })?.summary;
...
export function extractSummary(event: TelemetryEvent): ConsumptionSummary[] {
  ...
  const rawData = raw as { edata?: { summary?: unknown }; summary?: unknown };
  const rawSummary = (rawData?.edata?.summary ?? rawData?.summary) as
    | ConsumptionSummary
    | ConsumptionSummary[]
    | undefined;
...
/** Coerces a SCORM ASSESS event's string-typed score/maxscore into numbers before it is aggregated or sent on. */
