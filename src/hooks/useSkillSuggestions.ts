 ...
 *
 * Unenrolled paths need their hierarchy fetched — search results carry no
 * skill tags, so search alone can't answer "what would this teach me?". That
 * fan-out shares cache keys with `useCollection`/`useMySkills`, so a path
