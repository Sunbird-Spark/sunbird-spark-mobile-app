...
export abstract class BaseClient implements IHttpClient {
  ...
  protected onResponse(response: ApiResponse<unknown>): void {
  ...
  protected abstract _post<T>(url: string, data: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>>;
  protected abstract _put<T>(url: string, data: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>>;
  protected abstract _patch<T>(url: string, data: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>>;
  ...
  public async post<T>(url: string, data: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>> {
    const response = await this._post<T>(url, data, headers);
    this.onResponse(response);
    return response;
  }

  public async put<T>(url: string, data: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>> {
    const response = await this._put<T>(url, data, headers);
    this.onResponse(response);
    return response;
  }

  public async patch<T>(url: string, data: unknown, headers?: Record<string, string>): Promise<ApiResponse<T>> {
