...
import type { PdfPlayerConfig, PdfPlayerMetadata } from './types';
...
const SCRIPT_SELECTOR = 'script[data-pdf-player-script]';

type ServiceCtor = typeof import('./PdfPlayerService').PdfPlayerService;

const metadata = (identifier = 'do_pdf_1'): PdfPlayerMetadata => ({
  identifier,
  name: 'A document',
  artifactUrl: 'doc.pdf',
});
...
describe('PdfPlayerService', () => {
  let PdfPlayerService: ServiceCtor;
  ...
  beforeEach(async () => {
    ...
    mockFetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      text: () => Promise.resolve(':root{--pdf-bg:#fff}\nbody{margin:0}\n.pdf-body-x{color:red}'),
    });
    vi.stubGlobal('fetch', mockFetch);
    vi.spyOn(console, 'error').mockImplementation(() => {});

    ({ PdfPlayerService } = await import('./PdfPlayerService'));
  ...
  async function buildConfig(identifier = 'do_pdf_1'): Promise<PdfPlayerConfig> {
  ...
  describe('script injection', () => {
    it('injects the player script and resolves the config once it loads', async () => {
      ...
      expect(injectedScripts()[0].getAttribute('src')).toBe(
        '/assets/pdf-player/sunbird-pdf-player.js',
      );
      ...
      expect(mockBuildPlayerContext).toHaveBeenCalledWith(undefined, { contentId: 'do_pdf_1' });
    });
    ...
    it('does not re-inject the script after a successful load', async () => {
      ...
      await new PdfPlayerService().createConfig(metadata('b'));
    ...
    it('rejects when the script fails to load', async () => {
      ...
      await expect(pending).rejects.toThrow('Failed to load sunbird-pdf-player script');
    });
  ...
  describe('createConfig', () => {
    it('builds the pdf side-menu configuration with sharing and printing disabled', async () => {
    ...
    it('forwards the caller context overrides to buildPlayerContext', async () => {
      ...
      const pending = service.createConfig(metadata('do_pdf_2'), overrides);
      fireOnScript('load');
      await pending;

      expect(mockBuildPlayerContext).toHaveBeenCalledWith(overrides, { contentId: 'do_pdf_2' });
    });
  });
  ...
  describe('createElement', () => {
    it('scopes the fetched CSS inside the wrapper rather than the document head', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/assets/pdf-player/styles.css');

      const styleEl = wrapper.querySelector('style[data-pdf-player-styles]');
      expect(styleEl).not.toBeNull();
      const css = styleEl!.textContent ?? '';
      expect(css.startsWith('@scope ([data-pdf-player-wrapper]) {')).toBe(true);
      expect(css).toContain(':scope{--pdf-bg:#fff}');
      expect(css).toContain(':scope{margin:0}');
      expect(css).not.toContain(':root');
      // A class that merely *contains* "body" must survive the rewrite.
      expect(css).toContain('.pdf-body-x{color:red}');

      expect(document.head.querySelector('style[data-pdf-player-styles]')).toBeNull();
    });

    it('builds a sized wrapper around the custom element carrying the config', async () => {
      const config = await buildConfig('do_pdf_9');
      const wrapper = await service.createElement(config);

      expect(wrapper.getAttribute('data-pdf-player-wrapper')).toBe('true');
      expect(wrapper.getAttribute('data-player-id')).toBe('do_pdf_9');
      expect(wrapper.style.width).toBe('100%');
      expect(wrapper.style.height).toBe('100%');

      const inner = wrapper.querySelector('sunbird-pdf-player');
      expect(inner).not.toBeNull();
      expect(inner!.getAttribute('data-player-id')).toBe('do_pdf_9');
    ...
    it('reuses the cached CSS for later elements instead of re-fetching', async () => {
      ...
      const second = await new PdfPlayerService().createElement(config);

      expect(mockFetch).toHaveBeenCalledTimes(1);
      expect(first.querySelector('style[data-pdf-player-styles]')).not.toBeNull();
      expect(second.querySelector('style[data-pdf-player-styles]')).not.toBeNull();
    });
    ...
    it('shares a single in-flight CSS request between concurrent elements', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledTimes(1);
      expect(a.querySelector('style[data-pdf-player-styles]')!.textContent).toContain(
        ':scope{margin:0}',
      );
      expect(b.querySelector('style[data-pdf-player-styles]')).not.toBeNull();
    });
    ...
    it('renders without a style tag when the CSS request is not ok', async () => {
      ...
      expect(wrapper.querySelector('style[data-pdf-player-styles]')).toBeNull();
      expect(wrapper.querySelector('sunbird-pdf-player')).not.toBeNull();
      expect(console.error).toHaveBeenCalledWith(
        'Failed to load pdf player styles:',
        expect.objectContaining({ message: 'Failed to fetch pdf player styles: 500' }),
      );
    });
    ...
    it('caches the failure so a second element does not re-fetch', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledTimes(1);
      expect(second.querySelector('style[data-pdf-player-styles]')).toBeNull();
    });

    it('exposes a no-op unloadStyles because styles live inside the wrapper', () => {
      expect(() => PdfPlayerService.unloadStyles()).not.toThrow();
    });
  });
  ...
  describe('event listeners', () => {
    ...
    beforeEach(async () => {
      vi.useFakeTimers();
      wrapper = await service.createElement(await buildConfig('do_pdf_1'));
      inner = wrapper.querySelector('sunbird-pdf-player') as HTMLElement;
    ...
    it('maps playerEvent details onto a PdfPlayerEvent', () => {
      ...
      expect(event.playerId).toBe('do_pdf_1');
    ...
    it('forwards document TelemetryEvent details to the telemetry callback', () => {
      ...
      telemetry({ eid: 'START', object: { id: 'do_pdf_1' } });

      expect(onTelemetryEvent).toHaveBeenCalledWith({ eid: 'START', object: { id: 'do_pdf_1' } });
    });
    ...
    it('uses the element itself and a default id when there is no inner player element', () => {
      ...
      expect(onPlayerEvent.mock.calls[0][0].playerId).toBe('pdf-player');
    });
    ...
    it('keeps forwarding this player\'s telemetry until its END event arrives', () => {
      ...
      telemetry({ eid: 'INTERACT', context: { contentId: 'do_pdf_1' } });
      expect(onTelemetryEvent).toHaveBeenCalledTimes(1);

      telemetry({ eid: 'end', object: { id: 'do_pdf_1' } });
      expect(onTelemetryEvent).toHaveBeenCalledTimes(2);

      telemetry({ eid: 'INTERACT', object: { id: 'do_pdf_1' } });
      expect(onTelemetryEvent).toHaveBeenCalledTimes(2);
    });
