 ...
 * confirmed via `/summary/v1/read`) the instant a write succeeds. A stale
...
export function computeCourseProgress(
  ...
  // waived either the whole course or every one of its leaves.
  // `completionPercentage` and `contentStatus` are left untouched here — only
  // the level rollup below and the UI's "Optional" badge act on this flag; the
  // server stays the source of truth for percentages.
  //
 ...
 * contributes a flat 0% and pins the Level below 100% forever, even though the
 * learner was never expected to attempt it — see bug: a fully-waived Level
 * could never unlock the outcome assessment or certificate. A Level whose
 * courses are ALL optional is reported as 100% (`total: 0` would otherwise read
 * as an empty/degenerate Level rather than a satisfied one).
 */
export function computeLevelProgress(
  ...
  const pct =
    total > 0
      ? round(requiredProgresses.reduce((sum, p) => sum + p.pct, 0) / total)
 ...
 *    unenrolled learner deep-linking to a Level screen (`LearningPathLevelPage`,
 *    which gates only on this status) saw every Level as `notStarted`
 *    (openable) rather than locked.
...
export function getResumeTarget(
  ...
  // unattempted one never becomes the resume target — a learner excused from a
  // leaf must not be sent back into it. The `lastReadContentId` branch above is
  // deliberately NOT filtered: a learner who chose to open an optional leaf
  // still resumes there.
