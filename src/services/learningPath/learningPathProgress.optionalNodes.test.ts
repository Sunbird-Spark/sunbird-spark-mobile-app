...
describe('optional_nodes — no-op regression guard', () => {
  // The live /summary/v1/read response returns optional_nodes: [] today —
...
describe('optional_nodes — course/level exclusion', () => {
  it('marks a course optional when its own id is in optional_nodes', () => {
    ...
    expect(computeCourseProgress(course1, new Map(), pathSummary).optional).toBe(true);
  });
  ...
  it('does not mark a course optional when only SOME of its leaves are optional', () => {
    ...
    expect(computeCourseProgress(course1, new Map(), pathSummary).optional).toBe(false);
  });

  it('does not mark a leaf-less course optional (leafIds guard, not a vacuous every)', () => {
    const leafless = { ...course1, leafIds: [], leafNodesCount: 3 };
    const pathSummary = pathSummaryWith(['do_unrelated_id']);
    expect(computeCourseProgress(leafless, new Map(), pathSummary).optional).toBe(false);
  });

  it('an optional id not present in this path does not shrink anything', () => {
    const pathSummary = pathSummaryWith(['do_unrelated_id']);
    expect(computeCourseProgress(course1, new Map(), pathSummary).optional).toBe(false);
    expect(computeLevelProgress(level1, new Map(), pathSummary)).toEqual({
      pct: 0,
      completed: 0,
      total: 1,
      doneCourses: 0,
  ...
  it('a partially-optional Level averages only the required courses', () => {
    ...
    const summaryByCollectionId = new Map<string, ViewerSummaryRecord>([
      [
        required!.identifier,
        { userId: 'u1', active: true, status: 2, progress: 1, contentStatus: {}, completionPercentage: 100 },
      ],
    ]);
    const pathSummary = pathSummaryWith([optionalCourse!.identifier]);
    // Only the required course counts — fully done, so the Level is 100%, not
    // the 50% a naive two-course average would have produced.
    expect(computeLevelProgress(level, summaryByCollectionId, pathSummary)).toEqual({
      pct: 100,
      completed: 1,
      total: 1,
      doneCourses: 1,
  ...
  it('an optional leaf the learner completed anyway is not penalised', () => {
    ...
    const result = computeCourseProgress(course1, new Map(), pathSummary);
    expect(result.optional).toBe(true);
    // completionPercentage/contentStatus math is deliberately untouched —
    // completing optional content still shows as done, it just isn't required.
    expect(result.pct).toBe(100);
  });

  it('unions optional ids carried by the per-course fan-out records', () => {
    const summaryByCollectionId = new Map<string, ViewerSummaryRecord>([
      [course1.identifier, { userId: 'u1', active: true, status: 1, progress: 0, optionalNodes: course1.leafIds }],
    ]);
    expect(computeCourseProgress(course1, summaryByCollectionId, pathSummaryWith([])).optional).toBe(true);
  });
});
...
describe('optional_nodes — outcome/certificate gating', () => {
  it('unlocks the outcome assessment when every level is complete or waived (via levelStatuses)', () => {
    const levelProgress = [{ pct: 60, completed: 0, total: 1, doneCourses: 0 }]; // incomplete by pct alone
...
describe('optional_nodes — getResumeTarget skips optional leaves', () => {
  it('skips an optional leaf and resumes at the next incomplete one', () => {
    const pathSummary = pathSummaryWith([course1.leafIds[0]!]);
    expect(getResumeTarget(model, pathSummary, [pathSummary])?.contentId).toBe(course1.leafIds[1]);
  });

  it('still resumes at lastReadContentId even if it happens to be optional (deliberate open)', () => {
    const pathSummary = { ...pathSummaryWith(course1.leafIds), lastReadContentId: course1.leafIds[0]! };
    expect(getResumeTarget(model, pathSummary, [pathSummary])?.contentId).toBe(course1.leafIds[0]);
  });

  it('when every leaf of a course is optional, resumes into the next course entirely', () => {
    const pathSummary = pathSummaryWith(course1.leafIds);
    const target = getResumeTarget(model, pathSummary, [pathSummary]);
