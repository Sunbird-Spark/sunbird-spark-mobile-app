...
function rand(a: number, b: number) { return randomBetween(a, b); }
function pickColor(count: number) { return (COLORS[randomInt(0, count)] || COLORS[0]) as string; }
    ...
    if (p > 0.28 && p < 0.72) {
      ...
      if (randomFloat() < intensity * 0.35) {
        particles.push({
          ...
          color: pickColor(COLORS.length), type: 'circle',
    ...
    if (p > 0.2 && p < 0.6) {
      ...
      if (randomFloat() < intensity * 0.4) {
        ...
        particles.push({
          ...
          alpha: rand(0.5, 0.9), decay: rand(0.018, 0.032), color: pickColor(COLORS.length),
          type: 'rect', life: 1,
        });
      }
    }
    ...
    if (p > 0.22 && p < 0.66) {
      ...
      if (randomFloat() < intensity * 0.28) {
        particles.push({
          ...
          alpha: rand(0.5, 0.85), decay: rand(0.012, 0.022), color: pickColor(3),
